...
def parse_money(value):
    ...
    if 'milh' in text.lower():
        mult = 1_000_000
        text = text.lower().replace('milhões', '').replace('milhao', '').replace('milhões', '')
    elif 'bilh' in text.lower():
        mult = 1_000_000_000
        text = text.lower().replace('bilhões', '').replace('bilhao', '').replace('bilhões', '')
...
def parse_serasa_text(text):
    ...
    # --- Serasa Score ---
    score_match = re.search(r'Serasa Score Empresas\s*\n.*?(\d{3})\s*\n?\s*(?:Risco mínimo|Baixo risco|Risco moderado|Alto risco)', text)
    if not score_match:
        score_match = re.search(r'probabilidade da empresa pagar suas contas em dia nos próximos 6 meses\s*\n\s*(\d{3})', text)
    if not score_match:
    ...
    rev_match = re.search(r'R\$\s*([\d\.,]+\s*(?:milhões|milhão|milhao|milhes|bilhões|bilhão|mil))\s*ao\s*ano', text)
    if rev_match:
    ...
    # --- Registration status ---
    status_match = re.search(r'Situa[çc][ãa]o Cadastral\s*\n?\s*\n?\s*(ATIVA|INATIVA|SUSPENSA|BAIXADA)', text)
    if not status_match:
    ...
    rec_match = re.search(r'(Baixíssimo Risco|Baixo Risco|Risco Moderado|Alto Risco)', text)
    if rec_match:
    ...
    # --- City/State ---
    city_match = re.search(r'(?:INDUSTRIAL|CENTRO|QUALQUER\s+BAIRRO)[^,]*,\s*([A-ZÇÁÉÍÓÚÃÕ\- ]+)\s*-\s*([A-Z]{2})', text)
    if not city_match:
        addr_match = re.search(r'Endereço:.*?,\s*([A-ZÇÁÉÍÓÚÃÕ\- ]+)\s*-\s*([A-Z]{2}),', text)
        if addr_match:
